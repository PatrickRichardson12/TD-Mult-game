...
var bulletWidth = 4;
var bulletLength = 4;
var shoot_delay = 500; //PLAYER CAN CHEAT THIS
...
window.onload = function () {
    ...
    socket.on('state', function (players, bulletsArray, enemyArray, powerUp_Array,info_Array) {
            info = info_Array;
...
function drawPlayer() {
    ...
    var playerr = sprite({  //player sprite
        ...
        height: playerLength+20,
        image: playerSprite,
        x: player.x,
        y: player.y,
        dir : player.dir,
        shift: 10,  //shift is something i made up since i wanted the IMG to be larger than its actual hitbox and we had to 'shift' the pic over a bit. *** make the shift half the size of your extention. I grew the player by 20 aka shift 10.
        thing: 'player',
        tickPerFrames: 20,
        loop : 20
    });
...
function game_Over() {
    if(info.firstGame){
        ...
        canvasContext.fillText('Prepare to Survive', canvas.width / 3 -45, canvas.height / 2);
        canvasContext.fillStyle = '#012D0F';
        canvasContext.fillText('Click to Begin', canvas.width / 3 +10, canvas.height / 3 * 2)
        return;
    }
...
function draw_powerUps() {
    if (powerUp.length > 0) {
        ...
        bombSprite.src = 'bomb.png';
        var bombb = sprite({  //player sprite
                           context: canvasContext,
                           width: 45,
                           height: 45,
                           image: bombSprite,
