...
io.on('connection', function (socket) {
    ...
    socket.on('bullet', function (data) {
        if (data.up && data.left) {
            bullets.push({ bulletY: data.y + playerLength / 2, bulletX: data.x + playerWidth / 2, dir: 'up-left' });
        }
        else if (data.up && data.right) {
            bullets.push({ bulletY: data.y + playerLength / 2, bulletX: data.x + playerWidth / 2, dir: 'up-right' });
        }
        else if (data.down && data.left) {
            bullets.push({ bulletY: data.y + playerLength / 2, bulletX: data.x + playerWidth / 2, dir: 'down-left' });
        }
        else if (data.down && data.right) {
            bullets.push({ bulletY: data.y + playerLength / 2, bulletX: data.x + playerWidth / 2, dir: 'down-right' });
        }
        else if (data.left) {
            bullets.push({ bulletY: data.y, bulletX: data.x + playerWidth / 2, dir: 'left' });
        }
        else if (data.up) {
            bullets.push({ bulletY: data.y + playerLength / 2, bulletX: data.x + playerWidth/ 1.15, dir: 'up' });
        }
        else if (data.right) {
            bullets.push({ bulletY: data.y + playerLength / 1.15, bulletX: data.x + playerWidth / 2, dir: 'right' });
        }
        else if (data.down) {
            bullets.push({ bulletY: data.y + playerLength / 2, bulletX: data.x, dir: 'down' })
        }
    });
    ...
    socket.on('disconnect',function(){
        var index = player_Id_Array.indexOf(socket.id.toString());
        if (index > -1)
              player_Id_Array.splice(index,1);
        delete players[socket.id];
        if (player_Id_Array.length == 0){
              info.gameOver = true;  // ordering here matters cause i was kinda lazy with reset_game()
              reset_game();
              info.firstGame = true;
        }
    });
...
setInterval(function () {
    if (info.firstGame)
            io.sockets.emit('state', null,null,null,null,info);
    if (!info.gameOver &&  !info.firstGame) {
        player_Gets_Pow(); //did anyone hit a powerup.
        move_bullets(); //this moves the bullets  and then check to see if bullets hit walls
        move_enemy();
        enemy();
        io.sockets.emit('state', players,bullets,enemies,powerUp,info);
    }
}, 1000 / 60);
